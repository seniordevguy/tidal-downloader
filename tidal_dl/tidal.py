...
class TidalAPI(object):
    ...
    def getArtistAlbums(self, id, includeEP=False):
        ...
        # Create a new list to store the filtered albums
        filtered_albums = []

        for album1 in data:
            audioModes = album1['audioModes']
            if "DOLBY_ATMOS" not in audioModes:
                filtered_albums.append(album1)
                continue

        process_albums = []

        # process albums
        albumsToProcess = self.processAlbums(filtered_albums)
        for album2 in albumsToProcess:
            for dataAlbum in filtered_albums:
                if (album2['albumId'] == dataAlbum['id']):
                    if (album2['process'] == True):
                        process_albums.append(dataAlbum)

        albums = list(aigpy.model.dictToModel(item, Album()) for item in process_albums)
        if not includeEP:
            return albums
    ...
    def processAlbums(self, data):
        albums = []

        for album in data:
            mqa = False
            hiResLossless = False
            if "MQA" in album['mediaMetadata']['tags']:
                mqa = True
            if "HIRES_LOSSLESS" in album['mediaMetadata']['tags']:
                hiResLossless = True
            ...
            albums.append({
                ...
                "imageUrl": self.getCoverUrl(album['cover']),
        ...
        response = requests.post(self.getApiUrl() + "/albums", data=data, headers=headers)
    ...
    def updatePath(self, albumId, path):
        url = self.getApiUrl() + "/album/" + str(albumId)
    ...
    def updateDownloadedTrackCount(self, albumId):
        url = self.getApiUrl() + "/album/tracks/" + str(albumId)
